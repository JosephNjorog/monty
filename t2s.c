...
/**
 * is_mydelim - checks if stream has delimitor char
 *
...
/**
 * get_wrdlength - gets the word length of cur word in str
 *
...
int get_wrdlength(char *str, char *delims)
{
	...
	while (*(str + i))
	{
		if (is_mydelim(str[i], delims))
			pending = 1;
		else if (pending)
		{
			wLen++;
		}
		if (wLen > 0 && is_mydelim(str[i], delims))
			break;
		i++;
	}
	return (wLen);
...
/**
 * get_wrdcount - gets the word count of a string
 *
